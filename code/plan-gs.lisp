...
;; slightly modified version of gs.lisp to work with strips planning;
...
(defun itgs (start &optional show-all-open extract-path-yes-no bfs-or-dfs)
  (let ((OPEN (list (make-node :id 0 :state start)))
    ...
    (loop
	  ...
	  (mapcar #'(lambda(x)
		  ...
		  OPEN)
	(progn
	  (format t "~A [~D+~D, ~D]" (node-state (car OPEN))
			      (node-depth (car OPEN))
			      (node-h-eval (car OPEN))
			      (node-parent (car OPEN)))
	  (terpri)
	  (format t "... OPEN cont. ...")
	  (terpri)))
	  ...
	  (progn
	    ...
	    (pprint node)
	    (terpri)(terpri)
	    (if extract-path-yes-no
		(return (extract-path node CLOSED))
	      (return (custom-result (node-state node))))
	    ))  ;; custom-result is to be written for each applic. domain
